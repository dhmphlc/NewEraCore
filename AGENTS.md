...
A landmark's type answers **two** questions, not one. `RegionStory` says whether the place belongs in this region; `SiteTerrain` says whether it could have been built on this ground — a dam wants water within reach, an airport wants open country. Leaving the second out put hydroelectric dams on dry flat land, which is worse than no landmark because it tells the player nothing was thought about. **Dry land is asked first, for every type** — the first version only asked the places that *wanted* water, so a hospital or a radio mast could be sited in mid-ocean; ground that suits nothing now holds nothing, and the cell is simply empty. `SiteTerrain` is declared in `world/history/` with permissive defaults so the package stays Bukkit-free and testable; `LandLookup.siteTerrain()` supplies the real answers from the biome key (ocean/river for water, "peaks"/"hills"/"slopes"/"windswept" for rugged), which needs no chunk loaded. The filter can never empty the candidate list — `RADIO_TOWER` has no story affinity and no ground requirement — so constraining the ground changes a site's *kind*, never its existence.
...
**Routes hug the terrain; sites do not.** A route is thousands of blocks long, so levelling it would need the terrain kilometres ahead — the one thing a chunk forbidden from loading its neighbours cannot know. A *bounded site* is different: `Airfield` is a fixed rectangle at fixed coordinates, so its platform height is a pure function of the seed (anchored to `World.getSeaLevel()`, the only global height available for free), every chunk agrees on it without asking, and `Earthworks` then cuts and fills each column to it. A runway is level from end to end whatever the ground was doing, which is what an aircraft requires and what earthworks are. `Earthworks.MAX_FILL` caps the embankment: an unbounded fill over a valley pours a hundred-block plinth, which reads as a bug where a gap reads as subsidence. Use this pattern for anything else that must be flat.
